...
function EditProduct() {
    ...
    async function fetchMyAPI() {
        ...
        if (queryString.parse(location.search).type === 'perfume') {
            data.perfume.map((item) => {
        ...
        else if (queryString.parse(location.search).type === 'makeup') {
        ...
        else if (queryString.parse(location.search).type === 'facialCare') {
            data.facialCare.map((item) => {
                if (queryString.parse(location.search).id === item._id) {
                    setAvatar(item.img)
                    setReviewImg(item.img)
                    setName(item.name)
    ...
    const updateProduct = async () => {
        ...
        if (name && type && price && currentQuantity && size && color && avatar) {
            ...
            const response = await fetch(` http://localhost:3030/admin/product/${queryString.parse(location.search).type}/${queryString.parse(location.search).id}`, config)
            const data = await response.json()
            if (data) {
